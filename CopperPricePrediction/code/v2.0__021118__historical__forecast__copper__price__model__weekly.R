# ********************************************************************************************************************************************
#                 Copper Price Forecast v2.0 - 02 November 2018 -- Production Code -- Historical Prices Prediction  
# ********************************************************************************************************************************************
...
for (cutoff_date in c('2018-05-31', '2018-06-30', '2018-07-31', '2018-08-30'))
{
  for (weeks_lag in 2:number_of_weeks){

    max_date <- as.Date(cutoff_date) + weeks(weeks_lag)
    
    # Copper Spot Price table
    main_df <- as.data.table(subset(copper_df, Date <= as.Date(cutoff_date)))
    ...
    cathode_free_market <- subset(cathode_free_market, Date <= as.Date(cutoff_date))
    ...
    temp_df <- subset(temp_df, Date <= as.Date(cutoff_date))
    ...
    heavy_wiew_1 <- subset(heavy_wire_1, Date <= cutoff_date)
    ...
    tc_df <- subset(tc_df, Date <= as.Date(cutoff_date))
    ...
    main_df <- as.data.table(subset(main_df, Date <= as.Date(cutoff_date)))
    main_df$Date <- as.character(floor_date(as.POSIXct(main_df$Date, format = "%Y-%m-%d"), 'week'))
    main_df <- main_df[, lapply(.SD, mean, na.rm = T), .SDcols = 'copper_price', by = c("Date")]
    main_df$target <- main_df$copper_price
    ...
    heavy_wire_1 <- merge(heavy_wire_1, heavy_scrap_df, by = 'Date', all.x = T)
    heavy_wire_1 <- merge(heavy_wire_1, scrap_df, by = 'Date', all.x = T)
    heavy_wire_1 <- subset(heavy_wire_1, Date <= cutoff_date)
    ...
    test <- main_df[1:weeks_lag]
    test <- test[order(Date)]
    test$prediction_date <- test$Date + weeks(weeks_lag)
    
    pred <- forecast(model, h = nrow(test), xreg = test[,xreg_features, with = F])
    test <- cbind(test, data.frame(prediction = data.frame(pred)$Point.Forecast))
    test$point.forecast <- numeric(0)
    test[1]$point.forecast <- test[nrow(test),]$target
    test$point.forecast <- test$point.forecast + test$prediction
    test[2:nrow(test)]$point.forecast <- test[2:nrow(test)]$prediction
    test$point.forecast <- cumsum(test$point.forecast)

    output <- rbind(output, data.frame(weeks_lag = weeks_lag, test[1:weeks_lag, c('prediction_date', 'point.forecast')]))
    message(cutoff_date, " --> ", weeks_lag)
  }
  
}

# Adding actual prices for error calcuation
actual_df <- as.data.table(copper_df)
actual_df$Date <- as.character(floor_date(as.POSIXct(actual_df$Date, format = "%Y-%m-%d"), 'week'))
actual_df <- actual_df[, lapply(.SD, mean, na.rm = T), .SDcols = 'copper_price', by = c("Date")]
output$prediction_date <- as.character(output$prediction_date)
output <- merge(output, actual_df, by.x= 'prediction_date', by.y = 'Date', all.x = T)
output$error <- (output$point.forecast - output$copper_price) * 100 / output$copper_price
...
output[, historic_pred_price:=mean(prediction), by = 'pred_month']
output <- output[, .SD[1], by = 'pred_month']
output <- subset(output, select = c("pred_month", "historic_pred_price"))
output <- output[,c("pred_month", "historic_pred_price")]
...
conn <- odbcDriverConnect(connection = connectionString)
sqlExecute(conn, "DELETE FROM COEA2A.CPP_Historic_Price_Forecasts")
sqlExecute(conn, "INSERT INTO COEA2A.CPP_Historic_Price_Forecasts VALUES (?, ?)", output)
close(conn)
